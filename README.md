...
`cascadeq` is a persistent, disk-backed FIFO queue for `[]byte` items. It runs entirely in memory under normal load and spills to disk only when the configured memory limits are reached — so it is fast when the queue is shallow and durable when it grows deep. Data in the queue is persisted to disk when the queue is closed and is available for reading when the queue is opened again.
